...
The Orb opens the command layer. Commands that mutate data create an `ai_action_proposal` first and only write to SQLite after confirmation. Inbox organization proposals show per-task target goal, project, priority, and effort before confirmation. Day-planning proposals validate calendar conflicts and only schedule inside open free windows.
